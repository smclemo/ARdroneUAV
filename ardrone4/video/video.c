...
static char *dev_name = (char*)"/dev/video2";
static enum io_method io = IO_METHOD_MMAP;
static int fd = -1;
unsigned int seq;
struct buffer *buffers;
struct img_struct* my_img;
...
static void process_image(struct img_struct* out_image)
{
	out_image->timestamp = util_timestamp();
	out_image->seq = seq;
	uyvyToGrey((unsigned char *)out_image->buf, (unsigned char *)frame_pointer, image_size/2);
...
static int read_frame(void)
{
	...
	case IO_METHOD_MMAP:
		...
		frame_pointer = buffers[buf.index].start;
	...
	case IO_METHOD_USERPTR:
		...
		frame_pointer = (void *) buf.m.userptr;
...
struct img_struct *video_CreateImage(int bytesPerPixel)
{
	...
	img->buf = (unsigned char*)malloc(WIDTH*HEIGHT*bytesPerPixel);
	return img;
}
