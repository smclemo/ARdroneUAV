...
C_RESULT video_codec_open( video_controller_t* controller, codec_type_t codec_type )
{
  ...
  controller->width           = 0;//was 0
  controller->height          = 0;//was 0
...
{
  vp_api_picture_t blockline = {(PixelFormat) 0 };
...
C_RESULT uvlc_decode_blockline( video_controller_t* controller, vp_api_picture_t* picture, bool_t* got_image )
{
	...
	bool stop = false;
	// At least a complete blockline is found
	while( stop == false && !controller->picture_complete && controller->in_stream.index < (controller->in_stream.used >> 2) )
	{
		uvlc_unpack_controller( controller );
		...
		if( !controller->picture_complete )
		{
			...
			macroblock  = &controller->cache_mbs[0];
			gobs        = &controller->gobs[controller->blockline];
			//printf(stdout,"Block1 %i \n",(int)gobs);
			//fprintf(stdout,"Block2 %i \n",(int)gobs->macroblocks);
			//TODO
			//if ((int)gobs > (int) gobs->macroblocks - 10000 && ((int)gobs->macroblocks > 0)){
...
static void video_realloc_buffers( video_controller_t* controller, int32_t num_prev_blockline )
{
  ...
  controller->gobs = (video_gob_t*) realloc(controller->gobs, i * sizeof(video_gob_t));//wasnot *100
  memset( controller->gobs, 0, i * sizeof(video_gob_t));//wasnot *100
  ...
  for(; i > 0; i-- )
  {
    ...
    gob->macroblocks = (video_macroblock_t*) realloc( gob->macroblocks, j * sizeof(video_macroblock_t));//wasnot *100
    memset( gob->macroblocks, 0, j * sizeof(video_macroblock_t));//wasnot *100
