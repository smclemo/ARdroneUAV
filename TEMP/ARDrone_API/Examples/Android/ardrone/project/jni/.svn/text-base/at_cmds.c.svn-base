...
static void send_command(int nb_sequence)
{
	...
	if (motion_info.state) {
		...
		// up/middle/down zone => gaz 800/0/-800 mm/s
		if (motion_info.x <= sw/3) {
			if (motion_info.y > 2*sh/3) {
				gaz = 800;
			}
			else if (motion_info.y > sh/3) {
				gaz = 0;
			}
			else {
				gaz = -800;
	...
	if (trackball_info.state || motion_info.state) {
		// orientation values in degrees
		pitch = -(orientation.values[2]+20)*500;
		if (pitch >= 25000) {
			pitch = 25000;
		}
		if (pitch <= -25000) {
	...
	if (current > ocurrent + 30) {
		overflow += current - ocurrent - 30;
	}
	ocurrent = current;

	/* dump command every 2s */
	if ((nb_sequence & 63) == 0) {
		INFO("seq=%d radgp(%d,%d,%d,%d) ui=0x%08x over30=%d\n", nb_sequence,
