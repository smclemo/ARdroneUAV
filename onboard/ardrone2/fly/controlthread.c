...
struct setpoint_struct {
  ...
  int 	locX; //x location of blob in frame
  int 	locY; //y location of blob in frame
} setpoint;
...
int ctl_Init(char *client_addr) 
{
  ...
  setpoint.h_max=200; //cm
  setpoint.h_min=40; //cm
  setpoint.throttle_hover=0.59;
  setpoint.throttle_min=0.30;
  setpoint.throttle_max=0.80;
  			
	//init pid pitch/roll 
	pid_Init(&pid_roll,  0.4,0,-0.05,0);//0.5
	pid_Init(&pid_pitch, 0.4,0,-0.05,0);//0.5
	pid_Init(&pid_yaw,   0.8,0.004,-0.065,0);//1.00
	pid_Init(&pid_h,     0.004,0.01,-0.002,1);//0.0005
...
//      adj_y     = pid_Calc(&pid_y,     0 - y,     att.dt);
	...
	if (motor[0]<setpoint.throttle_hover/3)
		motor[0] = setpoint.throttle_hover/3;
	if (motor[1]<setpoint.throttle_hover/3)
		motor[1] = setpoint.throttle_hover/3;
	if (motor[2]<setpoint.throttle_hover/3)
		motor[2] = setpoint.throttle_hover/3;
	if (motor[3]<setpoint.throttle_hover/3)
		motor[3] = setpoint.throttle_hover/3;
    //send to motors
...
void ctl_SetSetpoint(float pitch, float roll, float yaw, float h, int locX, int locY)
{
...
{
  ctl_SetSetpoint(pitch+setpoint.pitch, setpoint.pitch+pitch, yaw+setpoint.yaw, h+setpoint.h, setpoint.locX, setpoint.locY);
}
...
{
  logcnt2++;
  fprintf(fp,"%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%i,%i\n"
    //sequence+timestamp
    ,logcnt2
    ,att.ts   // navdata timestamp in sec
    //sensor data
