...
#define BUF_SIZE 11
float mag_x_buffer[BUF_SIZE];
float mag_y_buffer[BUF_SIZE];
float mag_z_buffer[BUF_SIZE];
int ind;
...
*/	
	...
	mag_x_buffer[ind] = nav.mag_x;
	mag_y_buffer[ind] = nav.mag_y;
	mag_z_buffer[ind] = nav.mag_z;
	ind = (ind+1)%BUF_SIZE;
	
	att->mag_x = util_median(mag_x_buffer, BUF_SIZE);
	att->mag_y = util_median(mag_y_buffer, BUF_SIZE);
	att->mag_z = util_median(mag_z_buffer, BUF_SIZE);
 
  //smooth out missing h samples
	if(abs(nav.h-last_h)>0.4 && last_ts-nav.ts<0.05) 
		att->h = last_h;
	else
	{  
		att->h  = nav.h;
