...
{
  static float32_t exp_1 = 0.3678794411714423412; // expf(-1.0f);
...
float32_t rate_limiter(float32_t input, float32_t old_output, float32_t rate_max)
{
  ...
  rate = input - old_output;
  
  if( f_abs( (float_or_int_t)rate ) > rate_max )
    output = old_output + f_set_clamp( (float_or_int_t)rate, (float_or_int_t)rate_max );
  else
    output = input;
