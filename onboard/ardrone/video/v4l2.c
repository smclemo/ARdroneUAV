...
struct point processFrame()
{
    for (;;) 
	{
		...
		void* data_pointer = compressFrame();
		if (data_pointer != NULL)
		{
			struct point blobLocation = findBlob(data_pointer, 91, 100, 130, 136, 200, 240);
			// left of centre is negative, above centre is negative
			struct point offset;
			offset.x = blobLocation.x - 160;
			offset.y = blobLocation.y - 120;
			return offset;
		}
    }
}
